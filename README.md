### 开源声明
   _**橙单已完整开源全部脚手架代码，包括工作流、在线表单、操作权限和数据过滤权限等组件。其完整性和成熟度明显高于售价几近万元的竞品，而我们则全部源码开放，没有一丝遮蔽。与此同时，我们还提供了令人印象深刻且完全免费的线上开发文档。**_ 欢迎加入我们的技术支持QQ群2 (883176267)，您的技术问题我们会尽快给出回复。如果橙单对您确有帮助，Fork 的同时，也请 Star 一下。
> 橙单作为一款低代码生成工具，我们没有为您准备任何现成的业务模块，如人事、供应商、出入库、订单等，因为这些模块您都可以根据自己的需求，用橙单快速生成。

### 最新功能
- 基础功能，全部模块均支持 Oracle，此前版本已支持 MySQL 和 PostgreSQL。
- 工作流，支持自由驳回到可途径的审批节点。
- 工作流，支持会签减签，原来只是支持加签，支持减签后实现会签功能的闭合。
- 工作流，任务转办指派，可以选择指派多人。
...
- 支持工单号自定义编码、工单撤销和终止、会签加签、待办已办转办、催办、抄送和传阅、保存草稿、撤回和驳回、历史任务、审批详情列表、包含候选组的审批人列表、附件上传下载等。
