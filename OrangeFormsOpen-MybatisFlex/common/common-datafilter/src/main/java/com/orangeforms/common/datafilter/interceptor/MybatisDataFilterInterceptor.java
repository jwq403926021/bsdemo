...
public class MybatisDataFilterInterceptor implements Interceptor {
    ...
    public void loadInfoWithDataFilter() {
        ...
        for (BaseDaoMapper<?> mapperProxy : mapperMap.values()) {
            ...
            if (proxy instanceof MapperInvocationHandler) {
                proxy = ReflectUtil.getFieldValue(proxy, "mapper");
                proxy = ReflectUtil.getFieldValue(proxy, "h");
            }
    ...
    private void loadTenantFilterData(Class<?> mapperClass) {
        ...
        for (Field field : fields) {
            if (field.getAnnotation(TenantFilterColumn.class) != null) {
                ...
                tenantInfo.setTableName(modelClass.getAnnotation(Table.class).value());
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        FlexStatementHandler handler = null;
        try {
            handler = (FlexStatementHandler) invocation.getTarget();
        } catch (Exception e) {
            handler = (FlexStatementHandler)
