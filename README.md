### 本期新功能
> 相比于普通开源脚手架的组件集成和 API 封装，橙单则提供了更为完整实用的工作流功能。代码全部经过严格测试，可极大降低开发者的学习成本。与此同时，我们提供了完全免费的线上文档和教学视频支持。QQ群和微信群中的相关问题，均会得到快速的回复和技术支持。
...
>  **重点解释！** 在项目开始，我们使用了基于在线表单的工作流，然而系统上线一段时间后，随着流程业务的复杂化，需要改为静态表单模式，并进行部分二次开发。有鉴于此，我们在该版本充分考虑了在线表单工作流向静态表单工作流升级的平滑性问题，并进行了合理的数据兼容性处理。
