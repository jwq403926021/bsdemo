### 开源声明
   _**橙单已完整开源全部脚手架代码，包括工作流、在线表单、操作权限和数据过滤权限等组件。其完整性和成熟度明显高于售价几近万元的竞品，而我们则全部源码开放，没有一丝遮蔽。与此同时，我们还提供了令人印象深刻且完全免费的线上开发文档。**_ 欢迎加入我们的技术支持QQ群3 (392442075)，您的技术问题我们会尽快给出回复。如果橙单对您确有帮助，Fork 的同时，也请 Star 一下。
> 橙单作为一款低代码生成工具，我们没有为您准备任何现成的业务模块，如人事、供应商、出入库、订单等，因为这些模块您都可以根据自己的需求，用橙单快速生成。

### 最新功能
- 基础功能，全部模块均支持 Oracle，此前版本已支持 MySQL 和 PostgreSQL。
- 工作流，支持自由驳回到可途径的审批节点。
- 工作流，完美实现减签，减签时可动态判断会签完成条件，满足条件将主动跳转下一节点。
- 工作流，任务转办指派，可以选择指派多人。
- 工作流，下一个任务的指派人，可以选择指派多人。
- 工作流，工单列表增加最后审批状态字段，每一步审批操作均会同步到工单字段，以便在流程结束时可以准确判断，当前流程是否正常的审批通过。
- 工作流，工单列表支持自定义工单编码字段，可以配置的编码规则为，前缀 + 精确时间(日时分秒) + 中缀 + 流水号(可指定宽度)。
- 在线表单，运行时性能大幅优化，表单渲染和在线业务操作所需的内置表数据查询，均改为缓存操作。
- 在线表单，可为在线表单字段指定编码规则，规则同上，同时还可以指定表单显示时计算，还是提交时计算。
- 在线表单，在线表单，完美支持多数据源，业务表、字典表和在线表单内置表均实现独立分库。
- 在线表单，上传下载字段支持阿里云OSS和腾讯云COS。
...
#### 技术支持
- 欢迎加入我们的技术交流 QQ 群，如遇任何使用中的问题我们都将第一时间为您答疑。群3:[![加入QQ群3](https://img.shields.io/badge/392442075-red.svg)](https://qm.qq.com/cgi-bin/qm/qr?k=cMMom9SdX1j57T_58WihLZ4TBrBpokrv&jump_from=webapi)
