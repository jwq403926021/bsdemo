...
- **工单管理**，支持完整的工作流工单管理，可为每个流程配置独立的工单菜单和管理页面，并与现有的数据过滤权限完美整合。
- **选择用户**，支持基于角色、部门、岗位、本部门岗位、上级部门岗位、同级部门岗位、指定部门岗位、本部门领导岗位、上级部门领导岗位等候选组。
...
- **数据安全**，严格且合理的数据安全校验，流程敏感数据 (如合同附件) 不会被越权访问和下载。工作流支持审批中数据和最终发布数据分离，防止审批中数据污染其他业务关联表的数据，并在审批完成后执行主表及各关联从表的数据同步。
